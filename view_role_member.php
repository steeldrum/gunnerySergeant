...
if ( !$roleMember = RoleMember::getMember( $memberId ) ) {
...
if ( isset( $_POST["action"] ) and $_POST["action"] == "Save Changes" ) {
  ...
  displayForm( array(), array(), $roleMember );
}
...
function displayForm( $errorMessages, $missingFields, $member ) {
  ...
  displayPageHeader( "View member role: " . $member->getValueEncoded( "firstname" ) . " " . $member->getValueEncoded( "lastname" ) );
...
?>
    <form action="view_role_member.php" method="post" style="margin-bottom: 50px;">
      <div style="width: 30em;">
        ...
        <label for="role"<?php validateField( "role", $missingFields ) ?>>Role *</label>
        <input type="text" name="role" id="role" value="<?php echo $member->getValueEncoded( "role" ) ?>" />
...
?>
    ...
    <div style="width: 30em; margin-top: 20px; text-align: center;">
      <a href="view_role_members.php?start=<?php echo $start ?>&amp;order=<?php echo $order ?>">Back</a>
    </div>
...
?>
    <p>Your changes have been saved. <a href="view_role_members.php?start=<?php echo $start ?>&amp;order=<?php echo $order ?>">Return to member list</a></p>
<?php
  displayPageFooter();
}
